...
class OssService
{
    ...
    public static function getAdminFileName($filename)
    {
        if (config('base.domain')) {
            $url = config('base.domain') . '/' . self::$filepath . '/' . $filename;
        } else {
     ...
     * @return string
     */
    public static function getNewApiFileName($filename){
        if(config('my.api_upload_domain')){
            $url = config('my.api_upload_domain').'/'.self::$filepath.'/'.$filename;
        }else{
            $url = ltrim(config('my.new_upload_dir'),'.').'/'.self::$filepath.'/'.$filename;
        }
        return $url;
    }
    /**
