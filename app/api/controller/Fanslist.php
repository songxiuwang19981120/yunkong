...
use app\api\service\FanslistService;
use app\api\model\Fanslist as FanslistModel;
use think\exception\ValidateException;
use SplFileInfo;


class Fanslist extends Common
{
    protected $noNeedLogin = ["Fanslist/getimage"];

    /**
     * @api {post} /Fanslist/index 01、首页数据列表
     * @apiGroup Fanslist
     * @apiVersion 1.0.0
    ...
    function index()
    {
        ...
        $field = '*';
        $orderby = 'fanslist_id desc';

        $res = FanslistService::indexList($this->apiFormatWhere($where,FanslistModel::class), $field, $orderby, $limit, $page);
    ...
    function getfollowinglist()
    {
        ...
        $url = 'http://47.245.30.4:9999/rest/VideoInfo/followerList';
        ...
        if ($info['result'] == 0) {
            $aweme_list = $info['data']['followers']; //视频列表
            if (!empty($aweme_list)) {
    ...
    function getfollowinglistfor($has_more, $max_time, $user_id, $sec_user_id, $member_id)
    {
        ...
        $info = json_decode($userinfo, true);
        if ($info['result'] == 0) {
            $aweme_list = $info['data']['followers']; //视频列表followers
            if (!empty($aweme_list)) {
                foreach ($aweme_list as $k => $v) {
                    $adddata['nickname'] = $v['nickname'];
                    $adddata['member_id'] = $member_id;
    ...
    function getimage()
    {
        $where['ifpic'] = 1;
        // $where['member_id'] = 20;
        $head_img = db('fanslist')->where($where)->field('uid,fanslist_id,avatar_thumb,head_image')->limit(10)->select()->toArray();
            ...
            foreach ($head_img as $k => $v) {
                if(empty($v['head_image'])){
                    ...
                    if ($avatar_hash != '6786ffc93d6a02f2b30a98ee94132937') {
                        $path = app()->getRootPath() . "public/uploads/xiazai/fensi";
                        $savepath = $path . "/" . $v['uid'] . '.png';
                        $imageurl = config('my.host_url') . "/uploads/xiazai/fensi/{$v['uid']}.png";
                    ...
                    // var_dump($data);die;
                    db('fanslist')->where('fanslist_id',$v['fanslist_id'])->update($data);
                }else{
                    db('fanslist')->where('fanslist_id',$v['fanslist_id'])->update(['ifpic'=>0]);
                }
            }
        } else {
            echo '没有要下载的头像或者昵称';
